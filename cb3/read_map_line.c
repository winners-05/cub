...
void fill_map_with_one(char **str)
{
   ...
   // g_allmap_with_spaces = str;
   //ft_putstr("|%s|",g_allmap_with_spaces[3]);
  // check_player(g_allmap_with_spaces);
...
void init_player(char x,int i, int j)
{
    ...
    else 
    {
        ...
        player.y = (j * TILE_SIZE) + (TILE_SIZE / 2);
         g_p = 1;
        if (x == 'N')
            player.rotationAngle =270 * (M_PI / 180);
        else if (x == 'S')
            player.rotationAngle =  90 * (M_PI / 180);
        else if (x == 'E')
