...
/*   Updated: 2021/01/26 19:17:07 by sel-fcht         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		cast_wall_hit(int g_col, float columnid)
{
	...
	float corr = rays[g_col].distance * cos(columnid - player.rotationAngle);
	float distprojplane = (g_width / 2) / tan(player.FOV / 2);
	float wallst = (TILE_SIZE / corr) * distprojplane ;

	//DDA(g_col, (g_height / 2) - (wallst / 2), g_col, (g_height / 2) + (wallst / 2));
